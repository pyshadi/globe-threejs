...
- **Node.js ≥ 20.0.0** (Node 18 is end-of-life and not supported)
...
## Local Development (Testing in VS Code)

Follow these steps to test the library from source locally in VSCode before publishing.

### Prerequisites

- [Node.js ≥ 20](https://nodejs.org/)
- [VS Code Live Server extension](https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer) — provides a local HTTP server so that texture and GeoJSON assets load correctly (the `file://` protocol is blocked by browser CORS restrictions)

### Steps

**1. Clone the repository and install dependencies**
```bash
git clone https://github.com/pyshadi/globe-threejs.git
cd globe-threejs
npm install
```
...
npm run build
```
This produces `dist/bundle.js` with the `Globe` class exposed as a UMD global.
...
<head>
  <meta charset="UTF-8" />
  <title>Globe – local test</title>
  <style>body { margin: 0; overflow: hidden; }</style>
</head>
<body>
  <!-- Three.js must be loaded before the bundle so the THREE global is available -->
  ...
  <script>
    const globe = new Globe({
      ...
      onLocationClick: function(info) { console.log(info); }
    });
...
- Right-click `index.html` in the Explorer panel and choose **Open with Live Server**.
- The globe opens in your browser at `http://127.0.0.1:5500`.

> **Tip:** After editing `src/globe.js`, re-run `npm run build` and Live Server will automatically reload the page.
